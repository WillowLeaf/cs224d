...
class RNNLM(NNBase):
    ...
    def __init__(self, L0, U0=None,
        ...
        NNBase.__init__(self, param_dims, param_dims_sparse, 
                        bptt=bptt, alpha=alpha, rseed=rseed)
    ...
    def _acc_grads(self, xs, ys, test=True):
        """
        ...
        if test:
            T = len(xs)
            h = zeros((T+1, self.hdim))
            y_hat = zeros((T, self.vdim))
            
            for t in xrange(T):
                h[t] = sigmoid(self.params.H.dot(h[t-1]) + self.sparams.L[xs[t]])
                y_hat[t] = softmax(self.params.U.dot(h[t]))
            
            gamma = zeros(self.hdim)
            for t in xrange(T-1, -1, -1):
                delta2 = y_hat[t].copy()
                delta2[ys[t]] -= 1
                self.grads.U += outer(delta2, h[t])
                delta1 = h[t] * (1 - h[t]) * self.params.U.T.dot(delta2)
                gamma = delta1 + h[t] * (1 - h[t]) * self.params.H.T.dot(gamma)
                self.grads.H += outer(gamma, h[t-1])
                self.sgrads.L[xs[t]] = gamma.copy()
            return
        ...
        for t in xrange(ns):    
            delta2 = ps[t].copy()
            delta2[ys[t]] -= 1
            self.grads.U += outer(delta2, hs[t])
            delta1 = hs[t] * (1 - hs[t]) * self.params.U.T.dot(delta2)
            self.grads.H += outer(delta1, hs[t-1])
            self.sgrads.L[xs[t]] = delta1.copy()
            for s in xrange(t-1, max(t-self.bptt, -1), -1):                
                delta1 = hs[s] * (1 - hs[s]) * self.params.H.T.dot(delta1)                
                self.grads.H += outer(delta1, hs[s-1])
                self.sgrads.L[xs[s]] = delta1.copy()
    ...
    def deprecated_grad_check(self, x, y, outfd=sys.stderr, **kwargs):
        """
    ...
    def compute_seq_loss(self, xs, ys):
        ...
        for t in xrange(len(xs)):
            ...
            J += -log(softmax(self.params.U.dot(h))[ys[t]])
