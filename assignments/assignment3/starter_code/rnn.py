...
class RNN:
    ...
    def forwardProp(self, node, correct=[], guess=[]):
        cost  =  total = 0.0 
        # cost should be a running number and total is the total examples we 
        # have seen used in accuracy reporting later 
        ...
        if node.isLeaf:
            ...
            left_res = self.forwardProp(node.left, correct, guess)
            right_res = self.forwardProp(node.right, correct, guess)
            cost += left_res[0] + right_res[0]
            total += left_res[1] + right_res[1]
            z1 = self.W.dot(np.concatenate((node.left.hAct1, 
                                            node.right.hAct1))) + self.b
            node.hAct1 = np.maximum(0, z1)
            
        node.probs = softmax(self.Ws.dot(node.hAct1) + self.bs)
        node.fprop = True
    ...
    def backProp(self, node, error=None):
        ...
        deltas = node.probs.copy()
        deltas[node.label] -= 1.0
        
        self.dWs += np.outer(deltas, node.hAct1)
        self.dbs += deltas
        
        delta1 = self.Ws.T.dot(deltas)
        ...
        if node.isLeaf:
            self.dL[node.word] += delta1
        else:            
            self.dW += np.outer(delta1, np.concatenate((node.left.hAct1, 
                                                        node.right.hAct1)))
            self.db += delta1
            error_below = self.W.T.dot(delta1)
            self.backProp(node.left, error_below[:self.wvecDim])
            self.backProp(node.right, error_below[self.wvecDim:])
