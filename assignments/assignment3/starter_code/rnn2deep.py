...
class RNN2:
    ...
    def forwardProp(self, node, correct=[], guess=[]):
        ...
        if node.isLeaf:
            ...
            res_left = self.forwardProp(node.left, correct, guess)
            res_right = self.forwardProp(node.right, correct, guess)
            cost += res_left[0] + res_right[0]
            total += res_left[1] + res_right[1]
            z1 = self.W1.dot(np.concatenate((node.left.hAct1, 
                                            node.right.hAct1))) + self.b1
            node.hAct1 = np.maximum(0, z1)
        
        node.hAct2 = np.maximum(0, self.W2.dot(node.hAct1) + self.b2)
        node.probs = softmax(self.Ws.dot(node.hAct2) + self.bs)
    ...
    def backProp(self,node,error=None):
        ...
        self.dWs += np.outer(deltas, node.hAct2)
        self.dbs += deltas
        
        delta2 = (node.hAct2 != 0) * (self.Ws.T.dot(deltas))
        self.dW2 += np.outer(delta2, node.hAct1)
        self.db2 += delta2
        ...
        if node.isLeaf:
            ...
            self.dW1 += np.outer(delta1, np.concatenate((node.left.hAct1, 
                                                         node.right.hAct1)))
            self.db1 += delta1        
            error_below = self.W1.T.dot(delta1)
