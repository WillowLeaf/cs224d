...
def run(args=None):
    ...
    # Testing
    if opts.test:
        test(opts.inFile, opts.data, opts.model, plot=True)
        return
    ...
    if evaluate_accuracy_while_training:
        ...
        plt.plot(dev_accuracies, 'b')
        plt.savefig("train_dev_curves.png")
        plt.show()

def test(netFile, dataSet, model='RNN', trees=None, plot=False):
    if trees==None:
